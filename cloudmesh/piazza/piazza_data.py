...
class PiazzaData:
    ...
    def get_activity(self, sort=''):
        ...
        num_posts = sorted(num_posts, key=lambda k: (k[sort] if sort in k else k['posts'] + k['comments']),
                           reverse=True)
        ...
        for post in self.data:
            ...
            if comments:
                ...
                for comment in self.iter_comments(post):
                    ...
                    print(
                        indent + 'Author: ' + comment['name'] if 'name' in comment else indent + 'Author: (anonymous)')
                    ...
                    content = [content[i:i + length].strip() + '\n' + indent for i in
                               range(length - offset, len(content), length)]
    ...
    def print_history(self, detailed=False, comments=False, author=''):
        ...
        if comments:
            ...
            print()
            'User has {num} posts (including comments):'.format(num=len(user_posts))
            for post in user_posts:
                if detailed:
                    ...
                    if post['post_type'] == 'post':
                        print('Posted "' + self.clean_html(post['subject']) + '" in ' + ', '.join(
                            post['folders']) + ' on ' + self.convert_date(post['created']))
                    elif post['post_type'] == 'comment':
                        print('Commented on post "' + self.clean_html(post['parent_subject']) + '" in ' + ', '.join(
                            post['folders']) + ' on ' + self.convert_date(post['created']))
        else:
            print()
            'User has {num} posts:'.format(num=len(self.data))
            for post in self.data:
                if detailed:
                    print('Subject: ' + self.clean_html(post['subject'])
                    print('Author: ' + post['author'])
    ...
    def print_participation(self, folder, users, detailed=False, posted=''):
        ...
        """
        students = [{'uid': u['id'], 'name': u['name'], 'posts': 0, 'comments': 0} for u in users if
                    u['role'] == 'student']
        ...
        print('"{folder}" student participation: {percent}%'.format(folder=folder, percent=round(
            (float(len([k for k in students if k['posts'] or k['comments']])) / len(students)) * 100)))
