...
class PiazzaHandler:
    ...
    def get_uid(self, user):
        ...
        if len(users) == 0:
            ...
            while True:
                ...
                selection = raw_input(
                    'Enter number of user you are looking for ({nums}): '.format(nums='0-' + str(len(users) - 1)))
    ...
    def show(self, visual='', folder='', chart=''):
        ...
        if visual_trimmed == 'wordcloud':
            ...
            d = PiazzaData(self.mongo.find('posts')).class_participation().chart(chart_type, opts={'height': 400},
    ...
    @no_flask
    def search(self, users=False, post=False, subject=False, everything=False, comment=False, query='', user='',
               uid=''):
        """Search for query
        Args:
        ...
        if subject:
            ...
            d = PiazzaData(self.mongo.find('posts', search, {'_id': 0})).print_posts(highlight='subject',
                                                                                     highlight_text=query)

        if post:
            search = {'author_id': author_id} if author_id else {}
            search['content'] = regex
            d = PiazzaData(self.mongo.find('posts', search, {'_id': 0})).print_posts(highlight='content',
                                                                                     highlight_text=query)

        if comment:
            d = PiazzaData(self.mongo.find('posts', {}, {'_id': 0})).search_comments(query,
                                                                                     author=author_id).print_comments(
                highlight='subject', highlight_text=query)
    ...
    def participation(self, folder='', detailed=False, posted=''):
        ...
        """
        d = PiazzaData(self.mongo.find('posts', {'folders': folder})).print_participation(folder, self.mongo.find(
            'piazza_users'), detailed=detailed, posted=posted)
