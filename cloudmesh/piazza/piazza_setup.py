...
class PiazzaSetup:
    ...
    def needs_install(self):
        ...
        if not os.path.isfile(os.path.expanduser('~/piazza/piazza.cfg')):
            print('"piazza.cfg" does not exist. Entering install.')
            return True
        else:
            return False
    ...
    def needs_setup(self):
        ...
        """
        m = PiazzaMongo()
        if (not m.collection_in_db('posts') or not m.collection_in_db('meta') or not m.collection_in_db(
                'piazza_users')):
    ...
    def install(self):
        ...
        """
        install = self.get_y_n(
            'Do you wish to install Piazza Miner to {folder}'.format(folder=os.path.expanduser('~/piazza')))
        if install:
            self.move_files()
            self.modify_config()
    ...
    def modify_config(self):
        ...
        """
        modify = self.get_y_n(
            'Do you wish to modify "piazza.cfg"? NOTE: Information provided will be stored in the piazza.cfg file.')
